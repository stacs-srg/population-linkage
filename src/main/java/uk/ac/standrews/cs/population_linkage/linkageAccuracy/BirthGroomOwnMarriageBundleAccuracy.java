...
public class BirthGroomOwnMarriageBundleAccuracy extends AbstractAccuracy {
    ...
    private static final String BIRTH_GROOM_SIBLING_TPC = "MATCH (b:Birth)-[r:GROOM]-(m:Marriage) WHERE (b)-[:GROUND_TRUTH_BIRTH_GROOM_IDENTITY]-(m) return count(r)";
    private static final String BIRTH_GROOM_SIBLING_FPC = "MATCH (b:Birth)-[r:GROOM]-(m:Marriage) WHERE NOT (b)-[:GROUND_TRUTH_BIRTH_GROOM_IDENTITY]-(m) return count(r)";
    private static final String BIRTH_GROOM_SIBLING_FNC = "MATCH (b:Birth)-[r:GROUND_TRUTH_BIRTH_GROOM_IDENTITY]-(m:Marriage) WHERE NOT (b)-[:GROOM]-(m) return count(r)";
