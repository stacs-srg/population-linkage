...
//    private HashMap<String, List<LXP>> gt;
    ...
    public LinkageResult link(boolean pre_filter, boolean persist_links, boolean evaluate_quality, int numberOfGroundTruthTrueLinks, int prefilterRequiredFields, boolean generateMapOfLinks, boolean reverseMap) throws BucketException {
        ...
        ArrayList<LXP> filtered_source_records = filter(prefilterRequiredFields, NUMBER_OF_BIRTHS, linkageRecipe.getStoredRecords());
        ArrayList<LXP> filtered_search_records = filter(prefilterRequiredFields, EVERYTHING, linkageRecipe.getSearchRecords() ); // do not filter these - will reduce what we can find otherwise!

        linker.addRecords(filtered_source_records, filtered_search_records);
    ...
    public void showMarriage(LXP marriage) throws BucketException {
        ...
        String gid = marriage.getString(Marriage.GROOM_BIRTH_RECORD_IDENTITY);

        String bfirstname = marriage.getString(Marriage.BRIDE_FORENAME);
        String bsurname = marriage.getString(Marriage.BRIDE_SURNAME);
        String bid = marriage.getString(Marriage.BRIDE_BIRTH_RECORD_IDENTITY);

        long oid = marriage.getId();
        String std_id = marriage.getString( Marriage.STANDARDISED_ID );

        System.out.println(oid + "/" + std_id + ": G: " + gfirstname + "," + gsurname + " G Birth ID: " + gid + " B: " + bfirstname + "," + bsurname + " B Birth ID: " + bid );
    }
