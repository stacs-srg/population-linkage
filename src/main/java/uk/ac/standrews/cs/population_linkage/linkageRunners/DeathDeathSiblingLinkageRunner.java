...
import uk.ac.standrews.cs.population_linkage.linkageRecipies.DeathDeathSiblingLinkageRecipe;
...
public class DeathDeathSiblingLinkageRunner extends LinkageRunner {
    ...
    protected LinkageRecipe getLinkage(final String links_persistent_name, final String gt_persistent_name,
        ...
        return new DeathDeathSiblingLinkageRecipe(results_repository_name, links_persistent_name, gt_persistent_name, source_repository_name, record_repository);
    }
    ...
    protected Linker getLinker(final double match_threshold, final Metric<LXP> composite_metric, final SearchStructureFactory<LXP> search_factory) {
        return new SimilaritySearchLinker(search_factory, composite_metric, match_threshold, getNumberOfProgressUpdates(),
                "death-death-sibling", "threshold match at " + match_threshold, Death.ROLE_DECEASED, Death.ROLE_DECEASED, LinkagePostFilter::isViableBBSiblingLink);
    }
    ...
    public static void main(String[] args) {
        ...
        new DeathDeathSiblingLinkageRunner()
                .run("DeathDeathSiblingLinks", "DeathDeathSiblingGroundTruth",
