...
public class CreateGroundTruthUmeaSiblingBundling {
    ...
    public void run() throws Exception {
        ...
        List<Birth> all_records = createList(record_repository.getBirths());

        Map<String, List<Birth>> real_sibling_bundles = matchSiblingsUsingMarriageGroundTruth(all_records);                     // a map keyed on PARENT_MARRIAGE_RECORD_IDENTITY mapping to lists of births of siblings
        Map<String, RecordDistances> true_link_distances = populateTrueLinkRecordDistances(real_sibling_bundles);             // a map from concatenated record id to RecordDistances for all true links
        Map<String, RecordDistances> non_link_distances = populateFalseLinkRecordDistances(all_records, true_link_distances); // a map from concatenated record id to RecordDistances for chosen non links
    ...
    private List<Birth> createList(Iterable<Birth> births) {
    ...
    private Map<String, RecordDistances> populateFalseLinkRecordDistances(List<Birth> all_records, Map<String, RecordDistances> true_link_distances) {
        ...
        Map<String, RecordDistances> false_link_distances = new TreeMap<>();
     ...
     */
    private Map<String, List<Birth>> matchSiblingsUsingMarriageGroundTruth(Iterable<Birth> births) {

        Map<String, List<Birth>> map = new TreeMap<>();
    ...
    private Map<String, RecordDistances> populateTrueLinkRecordDistances(Map<String, List<Birth>> birth_map) {

        int count = 0;

        Map<String, RecordDistances> distances = new TreeMap<>();
     ...
     * @param births
     */
    private void addDistances(Map<String, RecordDistances> distances, List<Birth> births) {
    ...
    private void printFullSiblingBundles(Map<String, List<Birth>> birth_map) {
    ...
    private void printAbridgedSiblingBundles(Map<String, List<Birth>> birth_map) {
    ...
    private void printColumnHeaders(PrintStream out) {
        ...
        for (final NamedMetric<LXP> combined_metric : combined_metrics) {
            out.print(combined_metric.getMetricName() + DELIMIT);
        }
        out.println();
        out.flush();
    }
